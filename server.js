...
// Migrate existing sellers from listings table into sellers table
try {
  const existing = db.prepare("SELECT DISTINCT seller_email, seller_name, stripe_account_id FROM listings WHERE seller_email IS NOT NULL").all();
  for (const s of existing) {
    const already = db.prepare("SELECT id FROM sellers WHERE email = ?").get(s.seller_email);
    if (!already) {
      db.prepare("INSERT INTO sellers (id, name, email, stripe_account_id, stripe_onboarded) VALUES (?, ?, ?, ?, ?)").run(
        uuidv4(), s.seller_name || s.seller_email, s.seller_email,
        s.stripe_account_id || null, s.stripe_account_id ? 1 : 0
      );
    }
  }
...
// ── SELLER REGISTRATION (first time, from portal) ──
app.post('/seller/register', async (req, res) => {
  try {
    const { email, name } = req.body;
    if (!email || !name) return res.status(400).json({ error: 'Email and name required' });
    const emailClean = email.toLowerCase().trim();
    let seller = db.prepare("SELECT * FROM sellers WHERE email = ?").get(emailClean);
    if (!seller) {
      db.prepare("INSERT INTO sellers (id, name, email) VALUES (?, ?, ?)").run(uuidv4(), name.trim(), emailClean);
      seller = db.prepare("SELECT * FROM sellers WHERE email = ?").get(emailClean);
    }
    res.json({ success: true, seller: { id: seller.id, name: seller.name, email: seller.email, stripeOnboarded: !!seller.stripe_onboarded } });
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/seller/onboard', async (req, res) => {
  try {
    ...
    // If already onboarded, just return a new account link to re-confirm
    let accountId = seller.stripe_account_id;
    if (!accountId) {
      const nameParts  = name.trim().split(' ');
      const individual = { first_name: nameParts[0], last_name: nameParts.slice(1).join(' ') || 'Seller', email: emailClean };
      const account = await stripe.accounts.create({
        type: 'express', country: 'US', email: emailClean, business_type: 'individual', individual,
        ...
        metadata: { seller_name: name, seller_email: emailClean }
      });
      accountId = account.id;
      db.prepare("UPDATE sellers SET stripe_account_id = ? WHERE email = ?").run(accountId, emailClean);
    }

    const accountLink = await stripe.accountLinks.create({
      account: accountId,
      refresh_url: `${BASE_URL}/marketplace-seller-portal?reauth=true`,
      return_url:  `${BASE_URL}/marketplace-seller-portal?onboarded=true`,
...
app.get('/seller/status/:accountId', async (req, res) => {
  try {
    ...
    // Update onboarded flag in sellers table
    if (ready) {
      db.prepare("UPDATE sellers SET stripe_onboarded = 1 WHERE stripe_account_id = ?").run(req.params.accountId);
    }
    res.json({ ready, detailsSubmitted: account.details_submitted });
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/seller/magic-link', async (req, res) => {
  try {
    ...
    if (isStaff) {
      ...
      // Check sellers table OR existing listings
      const sellerRecord  = db.prepare("SELECT id FROM sellers WHERE email = ?").get(emailClean);
      const hasListings   = db.prepare("SELECT id FROM listings WHERE seller_email = ? LIMIT 1").get(emailClean);
      if (!sellerRecord && !hasListings) return res.status(404).json({ error: 'No account found. Please list an item first.' });

      // Auto-create seller record if they have listings but no seller record
      if (!sellerRecord && hasListings) {
        const l = db.prepare("SELECT seller_name, stripe_account_id FROM listings WHERE seller_email = ? LIMIT 1").get(emailClean);
        db.prepare("INSERT INTO sellers (id, name, email, stripe_account_id, stripe_onboarded) VALUES (?, ?, ?, ?, ?)").run(uuidv4(), l.seller_name || emailClean, emailClean, l.stripe_account_id || null, l.stripe_account_id ? 1 : 0);
      }
    }
...
app.get('/seller/portal', (req, res) => {
  try {
    const session = getSellerSession(req.query.token);
    ...
    res.json({
      ...
      seller: {
        email,
        name:            seller ? seller.name : (listings[0]?.seller_name || email),
        stripeOnboarded: seller ? !!seller.stripe_onboarded : false,
        stripeAccountId: seller ? seller.stripe_account_id : null
      },
      stats: {
...
// ── SELLER: COMPLETE ONBOARDING CALLBACK ──
app.post('/seller/onboarding-complete', async (req, res) => {
  try {
    const session = getSellerSession(req.body.token);
    if (!session) return res.status(401).json({ error: 'Invalid session' });
    const seller = db.prepare("SELECT * FROM sellers WHERE email = ?").get(session.email);
    if (!seller || !seller.stripe_account_id) return res.status(400).json({ error: 'No Stripe account found' });
    const account = await stripe.accounts.retrieve(seller.stripe_account_id);
    const ready   = account.charges_enabled && account.payouts_enabled;
    if (ready) db.prepare("UPDATE sellers SET stripe_onboarded = 1 WHERE email = ?").run(session.email);
...
// ── SELLER: CREATE LISTING FROM PORTAL ──
app.post('/seller/listings', upload.array('photos', 8), async (req, res) => {
  try {
    const session = getSellerSession(req.body.token);
    if (!session) return res.status(401).json({ error: 'Invalid session' });
    ...
    const { title, category, description, condition, price, listing_type, shipping_estimate } = req.body;
    if (!title || !price) return res.status(400).json({ error: 'Title and price required' });

    const isConcierge   = listing_type === 'concierge';
    const priceInCents  = Math.round(parseFloat(price) * 100);
    const shippingCents = Math.round(parseFloat(shipping_estimate || 0) * 100);
    const typeSafe      = listing_type || 'seller';

    if (priceInCents < 100) return res.status(400).json({ error: 'Minimum price is $1.00' });
    if (!isConcierge && !seller.stripe_onboarded) return res.status(400).json({ error: 'Please connect your bank account before listing.' });
    ...
    if (isConcierge) {
      ...
      if (cap.count >= CONCIERGE_CAPACITY) return res.status(400).json({ error: 'Concierge queue is currently full. Please check back soon.' });
    }
    ...
    db.prepare(`INSERT INTO listings (id,seller_name,seller_email,stripe_account_id,title,category,description,condition,price,shipping_estimate,photos,listing_type,dropoff_tier,status,concierge_status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
      .run(id, seller.name, seller.email, seller.stripe_account_id, title.trim(), category || 'Other', description || '', condition || 'Good', priceInCents, shippingCents, JSON.stringify(photos), typeSafe, 'self', status, isConcierge ? 'pending_intake' : null);
    ...
    if (isConcierge) {
      await sendEmail(seller.email, 'DBP Concierge — We Got Your Request!', emailTemplate('Concierge Intake Received',
        `<p style="font-size:15px;color:#2a1f0e;line-height:1.7;margin:0 0 16px;">Hi ${seller.name},</p>
         <p style="font-size:15px;color:#2a1f0e;line-height:1.7;margin:0 0 20px;">We received your concierge request for <strong>${title}</strong>. Drop it off at DBP (225 E 8th Ave) and we'll handle photos, listing, and shipping.</p>
         <div style="background:#e8dcc8;padding:16px 20px;margin-bottom:20px;">
           <p style="font-size:13px;color:#8a7a65;margin:0 0 4px;">Your estimated payout (65% of sale price)</p>
           <p style="font-size:24px;font-weight:700;color:#1d3a2e;margin:0;">~$${(priceInCents*0.65/100).toFixed(2)}</p>
         </div>`
      ));
      await sendEmail(NOTIFY_EMAIL, `[Concierge] New intake: ${title}`, `Seller: ${seller.name} · ${seller.email}\nItem: ${title}\nAsking: $${priceInCents/100}`);
    }

    const split = calculateSplit(priceInCents, shippingCents, typeSafe, 'self');
    res.json({ success: true, listingId: id, status, split, message: isConcierge ? 'Concierge request received. Drop off your item at DBP.' : 'Listing is live!' });
  } catch(err) { console.error('Seller listing error:', err); res.status(500).json({ error: err.message }); }
});
...
app.post('/staff/publish/:listingId', upload.array('photos', 8), async (req, res) => {
  try {
    ...
    // Use seller's stripe_account_id from sellers table
    const sellerRecord = db.prepare("SELECT stripe_account_id FROM sellers WHERE email = ?").get(listing.seller_email);
    const stripeId     = sellerRecord ? sellerRecord.stripe_account_id : listing.stripe_account_id;

    db.prepare(`UPDATE listings SET title=?,description=?,price=?,condition=?,category=?,shipping_estimate=?,photos=?,stripe_account_id=?,status='approved',concierge_status='listed',listing_type='concierge' WHERE id=?`)
      .run(title||listing.title, description||listing.description, priceInCents, condition||listing.condition, category||listing.category, shippingCents, JSON.stringify(photos), stripeId, req.params.listingId);
...
app.post('/admin/staff', (req, res) => {
  ...
  try {
    db.prepare("INSERT INTO staff (id, name, email) VALUES (?, ?, ?)").run(uuidv4(), name, email.toLowerCase().trim());
    res.json({ success: true });
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/admin/staff-commissions/mark-paid', (req, res) => {
  ...
  try {
    ...
    res.json({ success:true, markedPaid:result.changes });
  } catch(err) { res.status(500).json({ error: err.message }); }
});

// ── PUBLIC LISTINGS (kept for marketplace page) ──
app.post('/listings', upload.array('photos', 8), async (req, res) => {
  try {
    ...
    // Ensure seller record exists
    const emailClean = seller_email.toLowerCase().trim();
    const sellerExists = db.prepare("SELECT id FROM sellers WHERE email = ?").get(emailClean);
    if (!sellerExists) {
      db.prepare("INSERT INTO sellers (id, name, email, stripe_account_id, stripe_onboarded) VALUES (?, ?, ?, ?, ?)").run(uuidv4(), seller_name.trim(), emailClean, stripeId, stripeId ? 1 : 0);
    }
    ...
    res.json({ success:true, listingId:id, status, split, message:isConcierge?'Concierge request received.':'Listing submitted and live.' });
...
app.post('/alerts/subscribe', async (req, res) => {
  try {
    ...
    await sendEmail(email,'DBP Marketplace Alert Set Up',emailTemplate('Alert Confirmed ✓',
      `<p style="font-size:15px;color:#2a1f0e;line-height:1.7;margin:0 0 16px;">You're all set!</p>
       <div style="background:#e8dcc8;padding:14px 18px;margin-bottom:20px;">
         <p style="font-size:14px;color:#2a1f0e;margin:0 0 4px;"><strong>Category:</strong> ${category||'All'}</p>
         ${maxPrice?`<p style="font-size:14px;color:#2a1f0e;margin:0 0 4px;"><strong>Max price:</strong> $${maxPrice}</p>`:''}
         ${condition?`<p style="font-size:14px;color:#2a1f0e;margin:0;"><strong>Condition:</strong> ${condition}</p>`:''}
       </div>
       ...
       <p style="font-size:11px;color:#aaa;margin:20px 0 0;"><a href="${BASE_URL}/marketplace?unsubscribe=${unsubscribeToken}" style="color:#aaa;">Unsubscribe</a></p>`
    ));
    res.json({ success:true, message:"Alert set up!" });
  } catch(err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/checkout', async (req, res) => {
  try {
    ...
    // Get stripe_account_id from sellers table first, fall back to listing
    const sellerRecord = db.prepare("SELECT stripe_account_id, stripe_onboarded FROM sellers WHERE email = ?").get(listing.seller_email);
    const stripeAccountId = (sellerRecord && sellerRecord.stripe_account_id) || listing.stripe_account_id;
    const hasStripeAccount = stripeAccountId && stripeAccountId.trim() !== '' && stripeAccountId.startsWith('acct_');
    ...
    db.prepare(`INSERT INTO sales (id,listing_id,buyer_email,payment_intent_id,amount,seller_payout,dbp_payout,devo_payout,staff_payout,delivery_type) VALUES (?,?,?,?,?,?,?,?,?,?)`)
      .run(saleId,listingId,buyerEmail,paymentIntent.id,listing.price,Math.round(split.sellerNet*100),Math.round(split.dbpNet*100),Math.round(split.devo*100),Math.round(split.staff*100),deliveryType||'shipping');
...
app.post('/offers/:id/respond', async (req, res) => {
  try {
    ...
    const session=getSellerSession(token);
    if (!session) return res.status(401).json({ error: 'Invalid session' });
      ...
      await sendEmail(listing.seller_email,`Your ${listing.title} sold! 🎉`,emailTemplate('Item Sold!',
        `<p style="font-size:15px;color:#2a1f0e;margin:0 0 16px;">Hi ${listing.seller_name}, your <strong>${listing.title}</strong> just sold!</p>
         <div style="background:#e8dcc8;padding:16px 20px;margin-bottom:20px;">
           ...
           <p style="font-size:12px;color:#8a7a65;margin:8px 0 0;">${sale.delivery_type==='pickup'?'📍 Buyer will pick up at DBP.':sale.delivery_type==='detour'?'🚲 Detour delivery — drop off at DBP within 5 days.':'Transferred automatically 72 hours after delivery confirmation.'}</p>
         </div>
         <a href="${BASE_URL}/marketplace-seller-portal" style="display:inline-block;background:#c0531a;color:white;padding:14px 28px;font-size:14px;font-weight:600;text-decoration:none;">View in Portal →</a>`
      ));
      if (staffId&&sale.staff_payout>0) {
        const sm=db.prepare("SELECT * FROM staff WHERE id=?").get(staffId);
        if (sm) await sendEmail(sm.email,`Commission earned — ${listing.title} sold!`,emailTemplate('Commission Earned 🎉',`<p style="font-size:15px;color:#2a1f0e;margin:0 0 16px;">Hi ${sm.name}, an item you listed just sold!</p><div style="background:#e8dcc8;padding:16px 20px;"><p style="font-size:18px;font-weight:700;color:#1d3a2e;margin:0 0 8px;">${listing.title}</p><p style="font-size:28px;font-weight:700;color:#c0531a;margin:0;">$${(sale.staff_payout/100).toFixed(2)}</p><p style="font-size:12px;color:#8a7a65;margin:8px 0 0;">Paid monthly through DBP payroll.</p></div>`));
      }
    }
...
async function runBackup() {
  if (!R2_ENABLED) { console.log('Backup skipped — R2 not configured'); return { success:false }; }
  try {
    ...
    if (now.getDay()===0) {
      ...
      await sendEmail(NOTIFY_EMAIL,'DBP Weekly Backup',emailTemplate('Weekly Backup ✓',`<div style="background:#e8dcc8;padding:16px 20px;"><p style="margin:0 0 6px;"><strong>Listings:</strong> ${lc}</p><p style="margin:0 0 6px;"><strong>Sales:</strong> ${sc}</p><p style="margin:0;"><strong>Total paid out:</strong> $${(po/100).toFixed(2)}</p></div>`));
    }
...
async function releasePayouts() {
  try {
    const cutoff=new Date(Date.now()-72*60*60*1000).toISOString();
    const pending=db.prepare("SELECT s.*, l.stripe_account_id, l.staff_id FROM sales s JOIN listings l ON s.listing_id=l.id WHERE s.status='delivered' AND s.created_at<?").all(cutoff);
    for (const sale of pending) {
      try {
        if (process.env.DEVO_ACCOUNT_ID&&sale.devo_payout>0) await stripe.transfers.create({amount:sale.devo_payout,currency:'usd',destination:process.env.DEVO_ACCOUNT_ID,description:'Durango Devo — '+sale.listing_id,idempotency_key:`devo-${sale.id}`});
        db.prepare("UPDATE sales SET status='paid_out' WHERE id=?").run(sale.id);
...
function scheduleNightlyBackup() {
  ...
  setTimeout(()=>{ runBackup(); setInterval(runBackup,24*60*60*1000); }, next2am-now);
}
