...
app.get('/staff/portal', (req, res) => {
  try {
    ...
    const completed  = db.prepare(`
      SELECT l.*, s.staff_payout, s.staff_commission_paid, s.status as sale_status
      FROM listings l LEFT JOIN sales s ON s.listing_id = l.id
      WHERE l.staff_id = ? AND l.status IN ('approved','sold') ORDER BY l.created_at DESC
    `).all(staffMember.id).map(l => ({ ...l, photos: JSON.parse(l.photos || '[]'), price: l.price / 100, staff_payout: l.staff_payout ? l.staff_payout / 100 : null }));
...
app.get('/admin/staff-commissions', (req, res) => {
  ...
  try {
    ...
    const rows     = db.prepare(`
      SELECT st.id as staff_id, st.name, st.email, COUNT(s.id) as sales_count,
        SUM(s.staff_payout) as total_payout_cents,
        GROUP_CONCAT(l.title || ' ($' || ROUND(l.price/100.0,2) || ')') as items
      FROM sales s JOIN listings l ON s.listing_id = l.id JOIN staff st ON l.staff_id = st.id
      WHERE s.status IN ('delivered','paid_out') AND s.staff_commission_paid = 0
        AND DATE(s.created_at) BETWEEN ? AND ? AND s.staff_payout > 0
      GROUP BY st.id
    `).all(fromDate, toDate);
...
app.patch('/admin/listings/:id/stripe-account', (req, res) => {
  ...
  if (adminKey !== process.env.ADMIN_KEY) return res.status(401).json({ error: 'Unauthorized' });
  try {
    db.prepare("UPDATE listings SET stripe_account_id=? WHERE id=?").run(stripe_account_id || null, req.params.id);
    res.json({ success: true });
  } catch(err) { res.status(500).json({ error: err.message }); }
});
  ...
  try {
    ...
    if (now.getDay() === 0) {
      ...
      await sendEmail(NOTIFY_EMAIL, 'DBP Marketplace — Weekly Backup', emailTemplate('Weekly Backup ✓',
        `<div style="background:#e8dcc8;padding:16px 20px;">
           <p style="font-size:14px;color:#2a1f0e;margin:0 0 6px;"><strong>File:</strong> ${key}</p>
           <p style="font-size:14px;color:#2a1f0e;margin:0 0 6px;"><strong>Size:</strong> ${(dbBuffer.length/1024).toFixed(1)} KB</p>
           <p style="font-size:14px;color:#2a1f0e;margin:0 0 6px;"><strong>Listings:</strong> ${lc}</p>
           <p style="font-size:14px;color:#2a1f0e;margin:0 0 6px;"><strong>Sales:</strong> ${sc}</p>
           <p style="font-size:14px;color:#2a1f0e;margin:0;"><strong>Total paid out:</strong> $${(po/100).toFixed(2)}</p>
         </div>`
      ));
    }
